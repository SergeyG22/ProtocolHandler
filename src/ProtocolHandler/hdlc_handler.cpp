...
bool HDLC_Handler::checkSequenceforDuplicate(int current_index, int i) {
	std::vector<int> to_vector;
	std::copy(m_bit_buffer.begin() + current_index, m_bit_buffer.begin() + current_index + 6,
		std::back_inserter(to_vector));

	if (std::equal(to_vector.begin(), to_vector.end(), m_stuff_combination.begin(), m_stuff_combination.end())) {
		int drop_element_index = current_index - i + 8 + 1;
